...
// 29. https_request($url,$data = null)     GET,POST传参 curl调用接口链接返回状态
// 30. my_sys_uptime()                      获取 linux 服务器运行时间
// 31. get_area()                           淘宝接口：根据ip获取所在城市名称
// 32. bubbleSort($arr)                     冒泡排序 -- 数组升序排列
// 33. selectSort($arr)                     选择排序 -- 数组升序排列
// 34. insertSort($arr)                     插入排序 -- 数组升序排列
// 35. quickSort($arr)                      快速排序 -- 数组升序排列
// 36. heapSort($arr)                       堆排序 -- 数组升序排列
// 37. MergeSort($arr)                      归并排序将两个顺序序列合并成一个顺序序列的方法
// 38. json_encode_ex($json)						php中json_encode不兼容JSON_UNESCAPED_UNICODE的解决方案
// 39. 
// 40. 
// 
...
if (!function_exists('alert')) {
    /**
     * 38. PHP 使用JS alert() 弹窗提示信息
     * @param  [type] $msg [ 提示信息 ]
     * @return [type]      [ 执行js alert弹出信息提示 ]
     *
     * demo
     *
     * alert('123');
...
if (!function_exists('randomInt')) {
    ...
    function randomInt($num)
    {
        if (is_null($num) || empty($num) || !isset($num)) {
            ...
            $param = str_pad(mt_rand(0, 999999), $num, '0', STR_PAD_BOTH);
        }
        return $param;
    }
}
...
if (!function_exists('matchEmail')) {
     ...
     * demo
     *
     * var_dump(matchEmail('2929006594@qq.com'));
     */
...
if (!function_exists('matchPhone')) {
     ...
     * demo
     *
     * var_dump(matchPhone('17610900163'));
     */
...
if (!function_exists('Base64EncodeImage')) {
    /**
     * 42. 图片路径转Base64
     * @param [type] $ImageFile [ false/base64地址字串 ]
     *
     * demo
     *
...
if (!function_exists('newAddslashes')) {
    /**
     * 43. 返回经addslashes处理过的字符串或数组--对字串中的‘单引号’，‘双引号’，‘null’，‘反斜杠’进行转义
     * @param  [type] $string [description]
     * @return [type]         [description]
     *
     * demo
     * $string = 'hello "world"';
     * newAddslashes($string);
     */
...
if (!function_exists('newStripslashes')) {
    /**
     * 44. 返回经stripslashes处理过的字符串或数组 -- 删除反斜杠
     * @param  [type] $string [ 字符串/ 数组 ]
     * @return [type]         [ 删除 将数组/字符串 中反斜杠 ]
     *
     * demo
     * $string = 'hello \"world\"';
     * newStripslashes($string);
     */
...
if (!function_exists('newHtmlSpecialChars')) {
    /**
     * 45. 返回经htmlspecialchars处理过的字符串或数组 -- html 实体化处理
     * @param  [type] $string [ 需要处理的字符串或数组 ]
     * @param  [type] $charset [ 字符编码 ]
     * @return [type]         [ 处理后的字串 or 数组 ]
     *
     * demo
     *
     * $str = '"&lt;&copy; W3CS&ccedil;h&deg;&deg;&brvbar;&sect;&gt;";';
     * $result = newHtmlSpecialChars($str);
     * var_dump($result);
     */
    function newHtmlSpecialChars($string, $charset = 'utf-8')
    {
        if (strtolower($charset) == 'gbk') $charset = 'ISO-8859-15';
...
if (!function_exists('newHtmlEentityDdecode')) {
    /**
     * 46. 返回经html_entity_decode处理过的字符串或数组
     * @param  [type] $string  [需要处理的字符串或数组]
     ...
     * $result = newHtmlSpecialChars($str);
     *
     * var_dump(newHtmlEentityDdecode($result));
     *
     */
    function newHtmlEentityDdecode($string, $charset = 'utf-8')
    {
        if (strtolower($charset) == 'gbk') $charset = 'ISO-8859-15';
...
if (!function_exists('newHtmlentities')) {
    /**
     * 47. 返回经htmlentities处理过的字符串或数组
     * @param  [type] $string  [需要处理的字符串或数组]
     * @param  string $charset [字符编码]
     * @return [type]          [description]
     *
     ...
     *
     */
    function newHtmlentities($string, $charset = 'utf-8')
    {
        if (strtolower($charset) == 'gbk') $charset = 'ISO-8859-15';
...
if (!function_exists('safeReplace')) {
    /**
     * 48. 安全过滤函数
     * @param  [type] $string [ 待处理字串 ]
     * @return [type]         [ 替换字串中特殊字符 ]
     *
     * demo
     *
     * $str = 'hello "world" select * from userinfo';
     * var_dump(safeReplace($str));
     */
...
if (!function_exists('trimUnsafeControlChars')) {
    /**
     * 过滤ASCII码从0-28的控制字符
...
if (!function_exists('trimTextarea')) {
    /**
     * 格式化文本域内容 添加换行符
     * @param  [type] $string [ 文本域内容 ]
     * @return [type]         [ 换行处理后的内容 ]
     *
     * demo
...
if (!function_exists('formatJs')) {
    /**
     * 将文本格式成适合js输出的字符串(压缩至一行)
     * @param  [type]  $string [ 需要处理的字符串 ]
     * @param  integer $isjs [ 是否执行字符串格式化，默认为执行 ]
     * @return [type]          [ 将js代码压缩处理后的字符串 ]
     */
